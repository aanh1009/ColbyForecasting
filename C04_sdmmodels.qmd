...
Let's load what we need to build a model for August the model configuration. And we'll need the covariate data.  Notice that we select the covariates that are included in our configuration.
...
For each model type there is a convenient version designed to handle spatial data.
...
That's it! Of course, it's just a container of our goodies.  
...
So they are parameters (knobs) set for each model. These parameters help guide the optimal structure for solving the problem.  Think of a building contractor deciding which tool is the best one for the given task: "should I use my old hand saw? or should I use my laser guided power saw?"  For a model like Random Forest the structure might be "what is the best number of trees to use?" In order to figure out the best values to set these parameters to we actually vary them while running the model repeatedly and then picking the "best".    

### Iterate to find various hyperparameter sets

Next, we will deploy a function that will take each recipe-model combination and iterator over each of the many-many-many folds (well, 5 in our case) while adjusting the parameters (if any can be adjusted). The results of each iteration will be tracked so we can get a sense of the range and average.  And then we'll run each of those iterations three times and come up with an average for each.  Imagine that this is like a team of minions, each armed with a recipe and model, directing other minions to run the model fitting on each fold.  We are only setting up a grid of 3, which is fine for a class but in practice might be a much higher number.
...
autoplot(wflow)
```
Note that "default_glm" only has one result rather than three.  That is because there are no parameters to tune in general linearized models; it's one and done. 

### Choose the best set of hyperparamters for each model

What is the best configuration of hyperparameters for each model? Well, for "default_glm" there is only one, so that sort of doesn't count - we chose the only one we have. For the others we have to dig in to figure out which set of hyperparameters is best for each model.  We have written a wrapper function to speed things along; it accepts the workflow set and the path and filename to save the models.
...
We can make simple collective representations of the models.
...
```
And finally we can collate plots of Receiver Operator Curves (ROC) from which the Area Under the Curve (AUC) is computed.
...
```{r pd_plot}
model_fit_pdp(model_fits, id = "default_rf")
```

# Recap

We have built 4 tyoes of models using tools from the [tidymodels universe](https://www.tidymodels.org/).  After reading in a suite of data, we split our data into training and testing sets, withholding the testing set until the very end.  We looked a variety of metrics including a confusion matrix, ROC and AUC, accuracy and partial dependencies.  We saved the recipe and model together in a special container, called a workflow set, to a file.

# Coding Assignment

Use the [iterations tutorial](https://bigelowlab.github.io/handytandy/iterations.html) to build a workflow for each month using one or both of your background selection methods.  Save each workflow in the `models` directory.  You should end up with up to 12 workflows, but we know that observations can be highly seasonal so you may find you can't make models for some months.
