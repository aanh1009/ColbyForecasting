...
```{r tune}
wf = workflow_map(wflow, fn = "tune_grid")
```
...
This ROC is more typical of what we see in regular practice (although this looks low to a trained eye.)
...
```{r save_model}
write_workflow(fitted_wflow, version = cfg$version, type = get_model_type(fitted_wflow))
```
