...
obs = read_observations(scientificname = "Centropristis striata")
...
```{r save_model_input}
write_model_input(obsbkg, scientificname = "Centropristis striata")
```

Note that there is a function for reading model input files, too. 

```{r read_model_input}
x = read_model_input(scientificname = "Centropristis striata")
```
